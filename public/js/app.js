...
// Version checking for live updates
...
async function checkForUpdates() {
    try {
        ...
        if (currentAppVersion === null) {
            ...
            console.log('New version detected:', data.version, '(current:', currentAppVersion + ')');
            showUpdateNotification();
            // Stop checking once we've shown the notification
            stopVersionCheck();
        }
    } catch (error) {
...
function showUpdateNotification() {
    const toaster = document.getElementById('updateToaster');
    const updateBtn = document.getElementById('updateBtn');

    if (toaster) {
        toaster.style.display = 'block';

        // Check if demo creation is in progress
        const checkAndUpdate = () => {
            if (isCreating) {
                // Show disabled state
                updateBtn.classList.add('disabled');
                updateBtn.innerHTML = '<i class="fa-solid fa-hourglass-half"></i> Wait for Demo';
                return;
            }
            // Proceed with update
        ...
        // Update button state based on creation status
        const updateButtonState = () => {
            if (isCreating) {
                ...
                updateBtn.innerHTML = '<i class="fa-solid fa-rotate"></i> Update Now';
            }
        };
        ...
        const stateInterval = setInterval(updateButtonState, 1000);

        // Store interval ID for cleanup
        updateBtn.dataset.stateInterval = stateInterval;
    }
}
...
function startVersionCheck() {
    ...
    versionCheckInterval = setInterval(checkForUpdates, 30000);
}
...
// Start version checking when page loads
